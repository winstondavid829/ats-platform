...
### Current Issue Occurred
   Resume parsing is currently disabled for demo stability. The parsing service works, but due to intermittent network latency between services, it's temporarily disabled to ensure reliable application submissions. This can be re-enabled by setting DISABLE_RESUME_PARSING=false in the environment variables. Images are included under the project folder to show that scoring system works
